...
proc surveyreg data=dat.final;
		...
		domain include;
run;
data total_effect_pir (keep=parameter estimate probt lowercl uppercl include);
        set p;
		where substr(parameter, 1, 3) = "pir" AND include=1;
run;
...
data poorsleep_pir0100 (keep = replicate poorsleep_pir0100); set outcome_model;
        where parameter = 'poor_sleep*pir_cat Yes 0-100%' AND include=1;
...
data pir0100 (keep = replicate pir0100); set mediator_model;
        where parameter = 'pir_cat 0-100%' AND include=1;
        pir0100 = estimate;
run;
data pir199 (keep = replicate pir199); set mediator_model;
        where parameter = 'pir_cat 100-199%' AND include=1;
        pir199 = estimate;
run;
...
data short_sleep (keep = replicate short_sleep); set outcome_model;
        where parameter = 'short_sleep Yes' AND include=1;
...
data total_effect (keep=parameter estimate probt lowercl uppercl est_exp lwr_exp upr_exp);
        set total_effect_pir p;
		where (substr(parameter, 1, 3) = "DMD" OR substr(parameter, 1, 3) = "pir") AND include=1;
...
proc surveyreg data=outboot order=INTERNAL;* 1. outcome model;
		...
		domain include;
run;
data LessThan9th (keep = replicate LessThan9th); set mediator_model; where parameter = 'DMDEDUC2 Less Than 9th Grade' AND include=1;
LessThan9th = estimate; run;
data From9to11  (keep = replicate From9to11); set mediator_model; where parameter = 'DMDEDUC2 9-11th Grade (Includes 12th grade with no diploma)' AND include=1;
From9to11 = estimate; run;
data HighSchool (keep = replicate HighSchool); set mediator_model; where parameter = 'DMDEDUC2 High School Grad/GED or Equivalent' AND include=1;
HighSchool = estimate; run;
data SomeCollege (keep = replicate SomeCollege); set mediator_model; where parameter = 'DMDEDUC2 Some College or AA degree' AND include=1;
