...
proc format; * all formats;
	...
	value phys_act 1="Yes" 2="No" 3="Unable to activity";
run;
...
run;
proc freq data=nhanes; tables slq080 slq090 slq100 slq110 slq120; run;
...
data nhanes; *coding other covariates;
	...
	hrt = 0;
	if RHQ558 = 1 or RHQ566 = 1 or RHQ574 = 1 or RHQ584 = 1 or RHQ600 = 1 then hrt = 1;
	format hrt yesno.;
	...
	birth_control = .;
	if RHD442 = 1 then birth_control = 1;
	else if RHD442 = 2 then birth_control = 0;
	format birth_control yesno.;

	if PAD200 ge 7 then PAD200 = .;

	format RIAGENDR gender. RIDRETH1 race. PAD200 phys_act.;
...
data dat.final (keep=LBXCRP crp_bin sleep_dur poor_sleep pir_cat DMDEDUC2 agecat RIDAGEYR 
					cotinine_cat LBXCOT hrt obese sleep_med birth_control PAD200 RIAGENDR RIDRETH1 PAD200); 
	set nhanes;
run;
